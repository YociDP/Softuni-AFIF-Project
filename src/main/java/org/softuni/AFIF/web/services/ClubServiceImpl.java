...
import org.softuni.AFIF.web.error.ClubNotFoundException;
import org.softuni.AFIF.web.domain.entities.Club;
...
public class ClubServiceImpl implements ClubService {
    ...
    public ClubServiceModel findById(String id) {
        Club club = this.clubRepository.findFirstById(id);
        if(club == null)
        {
            throw new ClubNotFoundException("Club with given id does not exist!");
        }
        else {
            return this.modelMapper.map(club, ClubServiceModel.class);
        }
    }
